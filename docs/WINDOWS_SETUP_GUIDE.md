...
브라우저 창이 최대화되어 탭·주소 표시줄이 보이며 조회와 폼 입력까지 확인하고 저장 전 중단합니다. 13시만 확인하려면 `--hours 13`을 추가합니다. 예약 시간대가 활성화되지 않았으면 점검 실패가 정상일 수 있습니다. `--force`는 휴일만 무시하며 단독 사용 시 실제 저장이 가능합니다.
...
설정된 목표 시각까지 대기한 뒤 실제 저장을 수행합니다. 목표 시각이 지났으면 바로 진행합니다. 이전 `UNKNOWN`·`FAILED` 기록은 재시도를 막지 않습니다. 메뉴 5번과 스케줄러 실행에 동일하게 적용됩니다. 화면 없이 실행하려면 `--headless`를 사용합니다. 정기 실행만 사용할 경우 저장 없는 점검 후 다음 단계로 넘어갑니다.

영상도 필요하면 `run_automation.bat --headful --record-video`로 실행합니다. 메뉴 4·5번에서는 `config.json`의 `"record_video": true`로 켭니다(기본값 false). `headless=true`와 스케줄러 실행에서는 항상 녹화하지 않습니다. 이번 실행만 끄려면 `--no-record-video`를 사용합니다. 종료 후 각 시간대 폴더의 `video_*.webm`과 HTML에서 사이트 화면을 볼 수 있습니다. 브라우저 기본 확인·경고창은 영상에 포함되지 않으며 아래 PNG 캡처로 확인합니다.

브라우저 기본 확인·경고창까지 이미지로 보관하려면 화면 표시 모드인 메뉴 5번 또는 `--headful`을 사용합니다. 팝업 캡처 파일은 `log/날짜/시간대/*_browser_dialog_*.png`입니다. 캡처 후에는 자동으로 확인을 누릅니다. 추가 패키지 설치는 필요하지 않습니다.
