...
`log/YYYYMMDD/13/`에서 최신 `automation_*.html` 또는 `.log`를 열고 `[RESULT]`, `[FAIL]`, `[UNKNOWN]`, 통신 실패, `[COMPLETE]`를 검색합니다. 14·15시도 같은 방식입니다. 스크린샷은 같은 폴더에 있으며 실행 시각·재시도 번호가 파일명에 들어갑니다. 브라우저 기본 알림은 즉시 승인하므로 이미지에 남지 않을 수 있습니다. `브라우저 알림/팝업 즉시 승인` 로그의 메시지를 함께 확인하세요.

| 증상 | 확인 및 대응 |
|---|---|
| 이전 로그 경로나 result.json이 안 보임 | 현재는 log/날짜/시간대 아래 로그·HTML·PNG만 생성. 상태는 HTML의 [RESULT]·[COMPLETE]에서 확인. 이전 logs 폴더는 자동 삭제하지 않음 |
...
완료·대기 팝업 이미지는 같은 시간대 폴더의 `*_08_save_notice.png`를 확인하세요. 이후 `*_09_reservation_result.png`, `*_10_final.png`는 안내창을 닫은 뒤 화면입니다. 브라우저 기본 `alert/confirm`은 이 사이트 내부 안내 캡처와 별개이며 메시지가 로그에 남습니다.
