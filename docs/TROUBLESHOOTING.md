...
| 증상 | 확인 및 대응 |
|---|---|
| 이전 로그 경로나 result.json이 안 보임 | 실행 기록은 log/날짜/시간대에 통일. 상태는 HTML의 [RESULT]·[COMPLETE]에서 확인. 예약 상태 DB와 예전 DB 보관본은 .reservation_state에서 확인 |
...
| `[WAITING]` / `[RETRY]` | 서버 지연 대기 또는 읽기 전용 조회 재시도. 기본 조회 60초, 저장 120초. 저장은 재클릭하지 않음 |
| `NO_CHANGE` | 이미 품목 수 한도에 도달해 추가·저장을 생략한 결과 |
| `UNKNOWN` | 저장 응답 유실, 후속 대기 저장 실패, 예약번호 재조회 불일치 등. 다음 실행에서는 이전 기록에 막히지 않고 다시 저장 시도. [RESULT]와 서버 내역으로 결과 확인 |
| `SUBMITTING` | 같은 슬롯의 조회·예약 처리 중이거나 비정상 종료. 다른 프로세스와 서버 내역을 확인하고 운영자 검토 |
...
`UNKNOWN` 기록이 있어도 `run_automation.bat --headful --hours 13`으로 실행하면 실제 예약을 다시 시도합니다. 이전 오류 기록과 재시도 시작을 로그에 표시합니다. `--retry-unknown`은 호환용으로 유지하지만 지정할 필요가 없습니다. 로컬 확정·대기 기록을 지워 재저장하거나 DB 전체를 삭제하지 마세요. 현재는 로컬 `CONFIRMED`/`WAIT` 기록이 있어도 활성 슬롯의 품목 수를 다시 조회합니다. 여유가 있으면 추가하고 한도에 도달했으면 `NO_CHANGE`로 저장을 생략합니다. DB를 삭제할 필요가 없습니다. 자세한 규칙은 [운영 가이드](OPERATIONS_GUIDE.md)를 참고하세요.
