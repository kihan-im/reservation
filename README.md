...
실행 기록은 `log/`에 통일합니다. 중복 예약 방지용 내부 DB는 설정 파일 옆 `.reservation_state/reservation_state.sqlite3`에 별도 보관합니다. 이전 DB를 발견하면 기록을 가져오되 이미 새 위치에 있는 기록은 덮어쓰지 않습니다. 예전 `logs/`를 정리할 때는 기존 DB를 `.reservation_state/legacy_reservation_state.sqlite3` 등 별도 이름으로 보관하고, 최신 DB를 덮어쓰지 않습니다.

서버에 예약번호가 있다는 이유만으로 중단하지 않습니다. 해당 시간대 체크박스가 활성화되어 있으면 기존 품목을 불러와 미선택 자재를 최대 3개, 남은 허용 수량과 선택 가능한 품목 수 이내에서 추가합니다. 이전 로컬 `CONFIRMED`/`WAIT` 기록만으로 생략하지 않고, 매 실행마다 현재 체크박스와 서버 품목 수를 다시 확인합니다. 이미 품목 수 한도에 도달했으면 `NO_CHANGE`로 저장을 생략합니다. `UNKNOWN`과 `FAILED`는 다음 실행에서 별도 옵션 없이 다시 시도합니다. `SUBMITTING`은 같은 시간대의 다른 실행이 조회·예약을 처리 중임을 뜻하며 중복 실행을 막습니다. 각 탭은 사전 준비부터 목표 시각 대기·예약·결과 기록까지 독립 실행하므로 닫힌 시간대의 대기나 실패가 열린 시간대를 막지 않습니다. 같은 실행 안에서 저장 오류 후 반복 저장하지 않으며, 모든 탭의 사전 준비가 실패한 경우만 목표 시각 이전에 전체 재시작합니다.

미처리 시간만 재실행하려면 `--hours 14 15`처럼 지정합니다. `UNKNOWN`도 `--hours 13 --headful`처럼 일반 실행하면 체크박스 선택부터 실제 저장까지 다시 진행합니다. `--retry-unknown`은 이전 명령과의 호환을 위해 남겨 두었으며 이제 필요하지 않습니다. `SUBMITTING`은 실행 중인 프로세스가 없는지와 서버 내역을 확인한 후 운영 담당자가 해당 기록을 검토해야 합니다. 이 보호는 설정 파일 옆의 같은 `.reservation_state`를 공유하는 로컬 실행에 적용됩니다. 폴더를 옮기거나 DB를 삭제하여 중복 방지를 우회하지 마세요.
...
`site_timeout_seconds`는 조회·화면 로딩의 회당 대기 시간(기본 60초), `save_timeout_seconds`는 저장 응답·후속 대기 저장의 대기 시간(기본 120초)입니다. 조회만 429/502/503/504 또는 시간 초과 시 2초·4초 간격으로 최대 3회 시도합니다. 응답 지연은 10초마다 기록하고 저장 버튼은 재클릭하지 않습니다. 시간대 활성화 전체 대기는 `grid_wait_timeout_seconds`(기본 60초)로 제한하며, 이 안에서 목록 재조회 간격을 1→2→4초로 늘립니다.
